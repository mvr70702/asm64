/*  History:
    ...
    added code in eval to deal with #'>',">" etc.

ASM64 - MVR
    labelwarnings, printlable loop should end at '<= labelend'
    added DCI
    added mapfile
...
void endlist();
void liston(label *id,char **next);
void listoff(label *id,char **next);
...
char StringOnly[] = "Strings only in DCI";
char Pcoverrun[] = "PC out of (check)range";
...
int eval(char **str,int precedence,int type) {
    ...
    switch(unary) {
        ...
        case 39:                       // single quote MVR handle '>' etc
            s++;                       // point to char
            ret = *s; s++; s++;        // skip the char AND the closing quote
            break;
        case '#':
            s++;
            if(type==IMM) {
...
void labelwarnings(void) {
    ...
    for(i=labelstart;i<=labelend;i++) {   // original was <labelend
        p=labellist[i];

        if((*p).type==LABEL && (*p).referenced==0) {
            fprintf(stderr,"%s(%i): Unreferenced label `%s`[WARNING] 2\n",(*p).errsrc,(*p).errline,(*p).name);
        }
    }
}
...
void createmapfile(void)
{
  ...
  if (!mapfile)
  {
    fprintf(stderr,"Couldn't open mapfile %s\n\r",mapfilename);
    return;
  }
  for (i=labelstart;i<=labelend; i++) 
  {
    ...
    if (p != NULL)
    {
      ...
      {
        if (p->errsrc != NULL)
          fprintf(mapfile," %-16s  $%04x   %-20s:%4d       %s\n\r",p->name,p->value,p->errsrc,p->errline,typenames[p->type]);
        else  
          fprintf(mapfile," %-16s  $%04x    %36s\n\r",p->name,p->value,typenames[p->type]);
      }    
    }    
  }
  message("%s written\n",mapfilename);
...
void showhelp(void) {
    puts("");
    puts("asm64 " VERSION "\n");
    puts("Usage:  asm64 [-options] sourcefile [outputfile] [listfile] [mapfile]\n");
    puts("    -?          show this help");
    puts("    -l          create listing");
    puts("    -L          create verbose listing (expand REPT, MACRO)");
    puts("    -M          create mapfile");
...
int main(int argc,char **argv)
{
  ...
  for(i=1;i<argc;i++)
  {
    ...
    if (mapfilename == true_ptr)
    {
      strcpy(nameptr,".map");
      mapfilename=my_strdup(str);
    }
...
void ascii(label *id,char **next)
{
  dependant = 0;
//  charmap = asciimap;
}

void petscii(label *id,char **next)
{
  dependant = 0;
//  charmap = petsciimap;
}

void screen(label *id,char **next)
{
  dependant = 0;
//  charmap = screenmap;
}
...
void checkpc(label *id,char **next)
{
  int val;
